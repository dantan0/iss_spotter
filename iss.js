...
const fetchISSFlyOverTimes = function(coords, callback) {
  ...
  // console.log(latitude, longitude, 'hah');
  request(`http://api.open-notify.org/iss-pass.json?lat=${latitude}&lon=${longitude}`, (error, response, body) => {
    ...
    if (response.statusCode !== 200) {
      callback(Error(`Status Code ${response.statusCode} when fetching pass times for coordinates: ${body}`), null);
      return;
    }
...
const nextISSTimesForMyLocation = function(callback) {
  // const ip = fetchMyIP(callback);
  // const { latitude, longitude } = fetchCoordsByIP(ip, callback);
  // const passTimes = fetchISSFlyOVerTimes({ latitude, longitude }, callback);
  // return passTimes;
  fetchMyIP((error, ip) => {
    if (error) {
